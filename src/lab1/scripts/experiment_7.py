...
with open(base_path + 'savedmodels/json/exp_7.json', 'w') as json_file:
        json_file.write(finalmodel.to_json())
weights_file = base_path + 'savedmodels/weights/weights-MAMe-exp_7_'+str(preds_test[1])+'.hdf5'
...
plt.savefig(base_path + 'savedmodels/accuracy/exp_7_accuracy.pdf')
plt.close()
...
plt.xlabel('epoch')
plt.legend(['train','val'], loc='upper left')
plt.savefig(base_path + 'savedmodels/loss/exp_7_loss.pdf')
...
conf_fig.get_figure().savefig(base_path + 'savedmodels/conf/exp_7_confusion_matrix.pdf', dpi=400)