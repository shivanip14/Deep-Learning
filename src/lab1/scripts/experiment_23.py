...
with open(base_path + 'savedmodels/json/exp_23.json', 'w') as json_file:
        json_file.write(finalmodel.to_json())
weights_file = base_path + 'savedmodels/weights/weights-MAMe-exp_23_'+str(preds_test[1])+'.hdf5'
...
plt.savefig(base_path + 'savedmodels/accuracy/exp_23_accuracy.pdf')
plt.close()
...
plt.xlabel('epoch')
plt.legend(['train','val'], loc='upper left')
plt.savefig(base_path + 'savedmodels/loss/exp_23_loss.pdf')
...
conf_fig.get_figure().savefig(base_path + 'savedmodels/conf/exp_23_confusion_matrix.pdf', dpi=400)