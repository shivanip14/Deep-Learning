...
from tensorflow.keras.layers import Dropout, Input, concatenate, Dense, Conv2D, MaxPool2D, Flatten, UpSampling2D, BatchNormalization
...
model1 = MaxPool2D(pool_size=(2, 2), strides=(2, 2)) (model1)

model1 = Conv2D(256, (3, 3), activation='relu', padding="same") (model1)
model1 = BatchNormalization() (model1)
model1 = MaxPool2D(pool_size=(2, 2), strides=(2, 2)) (model1)

model1 = Conv2D(256, (3, 3), activation='relu', padding="same") (model1)
model1 = BatchNormalization() (model1)
model1 = UpSampling2D() (model1)

model1 = Conv2D(256, (3, 3), activation='relu', padding="same") (model1)
model1 = BatchNormalization() (model1)
model1 = UpSampling2D() (model1)

model2 = concatenate([model1, model])

model2 = Conv2D(512, (3, 3), activation='relu', padding="same") (model2)
model2 = MaxPool2D(pool_size=(2, 2), strides=(2, 2)) (model2)

model2 = Flatten() (model2)
model2 = Dense(100) (model2)
model2 = Dropout(0.7) (model2)
model2 = Dense(29, activation='softmax') (model2)

finalmodel = Model(inputs=input, outputs=model2)
finalmodel.summary()
...
with open(base_path + 'savedmodels/json/exp_10.json', 'w') as json_file:
        json_file.write(finalmodel.to_json())
weights_file = base_path + 'savedmodels/weights/weights-MAMe-exp_10_'+str(preds_test[1])+'.hdf5'
...
plt.savefig(base_path + 'savedmodels/accuracy/exp_10_accuracy.pdf')
plt.close()
...
plt.xlabel('epoch')
plt.legend(['train','val'], loc='upper left')
plt.savefig(base_path + 'savedmodels/loss/exp_10_loss.pdf')
...
conf_fig.get_figure().savefig(base_path + 'savedmodels/conf/exp_10_confusion_matrix.pdf', dpi=400)