...
model1 = Conv2D(32, (3, 3), activation='relu', padding="same") (model)
model2 = Conv2D(32, (3, 3), activation='relu', padding="same") (model)
...
model2 = Conv2D(64, (5, 5), activation='relu', padding="same") (model2)
...
model3 = Dense(29, activation='softmax') (model3)
finalmodel = Model(inputs=input, outputs=model3)
finalmodel.summary()
...
with open(base_path + 'savedmodels/json/exp_9.json', 'w') as json_file:
        json_file.write(finalmodel.to_json())
weights_file = base_path + 'savedmodels/weights/weights-MAMe-exp_9_'+str(preds_test[1])+'.hdf5'
...
plt.savefig(base_path + 'savedmodels/accuracy/exp_9_accuracy.pdf')
plt.close()
...
plt.xlabel('epoch')
plt.legend(['train','val'], loc='upper left')
plt.savefig(base_path + 'savedmodels/loss/exp_9_loss.pdf')
...
conf_fig.get_figure().savefig(base_path + 'savedmodels/conf/exp_9_confusion_matrix.pdf', dpi=400)