...
es_callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=1e-4, patience=10, restore_best_weights=True)

# Train
history = finalmodel.fit(mame_train_imgs, mame_train_labels, batch_size=32, epochs=100, validation_data=(mame_val_imgs, mame_val_labels), callbacks=[es_callback])
...
with open(base_path + 'savedmodels/json/exp_22.json', 'w') as json_file:
        json_file.write(finalmodel.to_json())
weights_file = base_path + 'savedmodels/weights/weights-MAMe-exp_22_'+str(preds_test[1])+'.hdf5'
...
plt.savefig(base_path + 'savedmodels/accuracy/exp_22_accuracy.pdf')
plt.close()
...
plt.xlabel('epoch')
plt.legend(['train','val'], loc='upper left')
plt.savefig(base_path + 'savedmodels/loss/exp_22_loss.pdf')
...
conf_fig.get_figure().savefig(base_path + 'savedmodels/conf/exp_22_confusion_matrix.pdf', dpi=400)