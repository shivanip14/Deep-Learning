...
from tensorflow.keras.layers import ELU, Input, Dropout, Dense, Conv2D, MaxPool2D, Flatten, GlobalAveragePooling2D, BatchNormalization
...
model = Conv2D(32, (3, 3), activation=ELU(), input_shape=(256, 256, 3), padding="same") (input)
model = Conv2D(32, (3, 3), activation=ELU(), padding="same") (model)
model = BatchNormalization() (model)
model = MaxPool2D(pool_size=(2, 2), strides=(2, 2)) (model)

model = Conv2D(64, (3, 3), activation=ELU(), padding="same") (model)
model = Conv2D(64, (3, 3), activation=ELU(), padding="same") (model)
model = BatchNormalization() (model)
model = MaxPool2D(pool_size=(2, 2), strides=(2, 2)) (model)

model = Conv2D(128, (3, 3), activation=ELU(), padding="same") (model)
model = Conv2D(128, (3, 3), activation=ELU(), padding="same") (model)
model = BatchNormalization() (model)
model = MaxPool2D(pool_size=(2, 2), strides=(2, 2)) (model)
