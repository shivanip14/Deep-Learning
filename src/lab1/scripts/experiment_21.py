...
# add image augmentation to both training and validation data
train_datagen = ImageDataGenerator(width_shift_range=0.1, height_shift_range=0.1, horizontal_flip=True)
test_datagen = ImageDataGenerator()

train_generator = train_datagen.flow(mame_train_imgs, mame_train_labels, batch_size=64, shuffle=False)
val_generator = train_datagen.flow(mame_val_imgs, mame_val_labels, batch_size=64, shuffle=False)
test_generator = test_datagen.flow(mame_test_imgs, batch_size=1, shuffle=False)
...
# Train
history = finalmodel.fit(train_generator, steps_per_epoch=(train_generator.n / 64), epochs=100, validation_data=val_generator, validation_steps=(val_generator.n / 64))
...
with open(base_path + 'savedmodels/json/exp_21.json', 'w') as json_file:
        json_file.write(finalmodel.to_json())
weights_file = base_path + 'savedmodels/weights/weights-MAMe-exp_21_'+str(preds_test[1])+'.hdf5'
...
plt.savefig(base_path + 'savedmodels/accuracy/exp_21_accuracy.pdf')
plt.close()
...
plt.xlabel('epoch')
plt.legend(['train','val'], loc='upper left')
plt.savefig(base_path + 'savedmodels/loss/exp_21_loss.pdf')
...
conf_fig.get_figure().savefig(base_path + 'savedmodels/conf/exp_21_confusion_matrix.pdf', dpi=400)
