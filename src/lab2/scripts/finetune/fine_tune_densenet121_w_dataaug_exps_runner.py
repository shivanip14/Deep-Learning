...
train_datagen = ImageDataGenerator(rescale=1. / 255, width_shift_range=0.1, height_shift_range=0.1, zoom_range=0.2)
val_datagen = ImageDataGenerator(rescale=1. / 255, width_shift_range=0.1, height_shift_range=0.1, zoom_range=0.2)
...
# loading DenseNet121 model from keras
weights_path = base_path + '/savedmodels/weights/densenet121-base-no-top.h5'
if delay_loading_weights:
    weights_path = None
loaded_model = DenseNet121(include_top=False, weights=weights_path, input_shape=(256, 256, 3))

print('Pre-trained DenseNet121 model:\n')
loaded_model.summary()

final_model = fine_tune_model(loaded_model, delay_loading_weights, base_model_exp)

print('Fine-tuned DenseNet121 model:\n')
final_model.summary()
